...
void InitRankingScore(void)
{
	...
	{
		pVtx[0].pos = D3DXVECTOR3(0.0f + nCntRanking * 24.0f, 180.0f, 0.0f);
		pVtx[1].pos = D3DXVECTOR3(20.0f + nCntRanking * 24.0f, 180.0f, 0.0f);
		pVtx[2].pos = D3DXVECTOR3(0.0f + nCntRanking * 24.0f, 220.0f, 0.0f);
		pVtx[3].pos = D3DXVECTOR3(20.0f + nCntRanking * 24.0f, 220.0f, 0.0f);
...
void UpdateRankingScore(void)
{
	...
	{//最も小さい値より挿入する値が大きかった時
	 ...
	 //最も小さい値に挿入する値を代入する
		g_aData[NUM_RANKING - 1] = GetScore();
	...
	for (nCount1 = 0; nCount1 < NUM_RANKING - 1; nCount1++)
	{
		...
		{
			if (g_aData[nCount2] > g_aData[nCount1])
			{//比較してる数が
...
void SetRanking(D3DXVECTOR3 pos, int nRanking)
{
	...
	for (nCntRanking = 0; nCntRanking < NUM_RANKING; nCntRanking++)
	{
		if (g_aRanking[nCntRanking].bUse == false)
		{
			...
			aTexU[0] = g_nRanking % 100000000 / 10000000;
			aTexU[1] = g_nRanking % 10000000 / 1000000;
			aTexU[2] = g_nRanking % 1000000 / 100000;
			aTexU[3] = g_nRanking % 100000 / 10000;
			aTexU[4] = g_nRanking % 10000 / 1000;
			aTexU[5] = g_nRanking % 1000 / 100;
			aTexU[6] = g_nRanking % 100 / 10;
			aTexU[7] = g_nRanking % 10;

			for (nCntNumber = 0; nCntNumber < NUM_PLASE; nCntNumber++, pVtx += 4)
			{
				g_aRanking[nCntNumber].pos = pos;

				pVtx[0].pos = D3DXVECTOR3(g_aRanking[nCntNumber].pos.x - 16.0f + (nCntNumber * 45.0f), g_aRanking[nCntNumber].pos.y - 25.0f, 0.0f);
				pVtx[1].pos = D3DXVECTOR3(g_aRanking[nCntNumber].pos.x + 16.0f + (nCntNumber * 45.0f), g_aRanking[nCntNumber].pos.y - 25.0f, 0.0f);
				pVtx[2].pos = D3DXVECTOR3(g_aRanking[nCntNumber].pos.x - 16.0f + (nCntNumber * 45.0f), g_aRanking[nCntNumber].pos.y + 25.0f, 0.0f);
				pVtx[3].pos = D3DXVECTOR3(g_aRanking[nCntNumber].pos.x + 16.0f + (nCntNumber * 45.0f), g_aRanking[nCntNumber].pos.y + 25.0f, 0.0f);

				pVtx[0].tex = D3DXVECTOR2(0.0f + (aTexU[nCntNumber] * 0.1f), 0.0f);
				pVtx[1].tex = D3DXVECTOR2(0.1f + (aTexU[nCntNumber] * 0.1f), 0.0f);
				pVtx[2].tex = D3DXVECTOR2(0.0f + (aTexU[nCntNumber] * 0.1f), 1.0f);
