...
void InitTime(void)
{
	...
	g_posTime = D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT - 200, 0);	//位置を初期化
	g_nTime = 300;														//値を初期化
	g_InitTime = 300;
...
void UpdateTime(void)
{
	...
	for (nCntScore = 0; nCntScore < NUM_PLACE; nCntScore++, pVtx += 4)
	{
		...
		pVtx[0].pos = D3DXVECTOR3(1170.0f + (nCntScore * 25.0f), 10.0f, 0.0f);
		pVtx[1].pos = D3DXVECTOR3(1190.0f + (nCntScore * 25.0f), 10.0f, 0.0f);
		pVtx[2].pos = D3DXVECTOR3(1170.0f + (nCntScore * 25.0f), 50.0f, 0.0f);
		pVtx[3].pos = D3DXVECTOR3(1190.0f + (nCntScore * 25.0f), 50.0f, 0.0f);
	}
	//頂点バッファのアンロック
	g_pVtxbuffTime->Unlock();
}
...
void SetTime(int nScore)
{
	...
	for (nCntScore = 0; nCntScore < NUM_PLACE; nCntScore++, pVtx += 4)
	{
		...
		//｜　テクスチュ座標の設定　｜
		//◇――――――――――――◇
		pVtx[0].tex = D3DXVECTOR2(0.01f + (aTexU[nCntScore] * 0.1f), 0.0f);
		pVtx[1].tex = D3DXVECTOR2(0.1f + (aTexU[nCntScore] * 0.1f), 0.0f);
		pVtx[2].tex = D3DXVECTOR2(0.01f + (aTexU[nCntScore] * 0.1f), 1.0f);
